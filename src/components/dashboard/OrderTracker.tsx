...
export default function OrderTracker({ status, updatedAt }: OrderTrackerProps) {
  ...
  // Mapping logic to handle various string inputs from the DB
  const currentIndex = steps.findIndex(step => {
    // Treat 'completed' or 'delivered' as the final step
    if (currentStatus === 'completed' || currentStatus === 'delivered') return step.id === 'delivered';
    // Treat 'paid' or 'pending' as the first step
    if (currentStatus === 'paid' || currentStatus === 'pending') return step.id === 'paid';
    // Match exact strings like 'preparing' or 'out-for-delivery'
    return currentStatus === step.id;
  });

  // Fallback to ensure at least the first step is active for any order
  const activeIndex = currentIndex === -1 ? 0 : currentIndex;

  return (
    <div className="w-full py-10 px-8 bg-white rounded-[3rem] border border-stone-100 shadow-sm">
      ...
      {/* Activity Timestamp */}
      {updatedAt && (
        <div className="flex items-center justify-center gap-2 pt-4 border-t border-stone-50">
